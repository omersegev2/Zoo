...
void Zoo::AddEnclosure(string id_enclosure){
    ...
    if (temp != NULL)
    {
        ...
        temp[num_of_enclosure - 1].Set_Id(id_enclosure);
        if (num_of_enclosure > 1){
            delete []this->enclosures;
        }
...
void Zoo:: RemoveAnimal(string name_animal, string id_enclosure){
    this->RemoveAnimal(this->FindAnimal(name_animal,id_enclosure),id_enclosure);
}
